...
export async function PrintPaySlip(
  ...
  const response = await BcApiService.fetchData<any>({
    url: `/ODataV4/HRMISActions_PrintPaySlip?Company=${companyId}`,
    method: "POST",
    data: {
      no: employeeNo,
      period: payrollId,
  ...
  if (response.data && response.data.value) {
    try {
      ...
      console.log(
        "First few bytes:",
        Array.from(byteCharacters.substring(0, 10)).map((b) => b.charCodeAt(0))
      );
      ...
      // Log the actual PDF content for debugging
      console.log(
        "PDF Content (first 100 chars):",
        byteCharacters.substring(0, 100)
      );
      console.log(
        "PDF Content (last 100 chars):",
        byteCharacters.substring(byteCharacters.length - 100)
      );
      ...
      if (byteCharacters.length < 1000) {
        ...
        console.log("Full PDF content:", byteCharacters);
      }
      ...
      throw new Error("Failed to convert PDF response");
    }
  } else {
