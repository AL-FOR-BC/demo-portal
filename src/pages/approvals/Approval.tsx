...
function Approvals() {
  ...
  const getApprovalPath = (documentType: string, documentNo: string) => {
    ...
    switch (type) {
      ...
      case "stores requisition":
        return `/approve-store-request/${documentNo}`;
      case "leave request":
        return `/approve-leave-request/${documentNo}`;
      case "leave plan":
  ...
  const columns = [
    ...
    },
    {
      ...
      formatter: (cell: any, row: any) => {
        ...
        console.log(
          "getApprovalPath:",
          getApprovalPath(row.DocumentType, row.DocumentNo)
        );
        return (
          <ActionFormatter
            row={row}
  ...
  const fetchApprovals = async (filterType = "") => {
    try {
      setIsLoading(true);
      let filterQuery = `$filter=(UserEmail eq '${email}' or UserEmail eq '${email}' or UserEmail eq '${email.toUpperCase()}' or UserEmail eq '${formatEmailFirstPart(
        email
      ...
      if (response.data?.value) {
        // map through each object and update the status key form Status to status
        const updatedApprovals = response.data.value.map((approval) => ({
          ...approval,
      ...
      const filterQuery2 = `$filter=(approverEmailAddress eq '${email}' or approverEmailAddress eq '${email}' or approverEmailAddress eq '${email.toUpperCase()}' or approverEmailAddress eq '${formatEmailFirstPart(
        email
      )}' or approverEmailAddress eq '${formatEmailDomain(
        email
      )}')&$expand=timeSheetLines`;

      const timeSheetApproval = await TimeSheetsService.getTimeSheetHeader(
        companyId,
        filterQuery2
      );
