...
import {
  ...
  setCompany,
} from "../../../../store/slices/auth/sessionSlice.ts";
...
function Header() {
  ...
  const { companyId, companies, allowCompanyChange } = useAppSelector(
    (state) => state.auth.session
  );
  const { settings } = useSettings();
  const { companyLogo } = settings;
