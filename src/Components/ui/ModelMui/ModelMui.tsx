...
const ModelMui: React.FC<ModelMuiProps> = ({
  ...
  const handleSubmitWithLoading = async () => {
    if (handleSubmit) {
      ...
      console.log("isSubmitting will be true (but not yet in this render)");
      try {
        await handleSubmit();
        console.log("handleSubmitWithLoading");
  ...
  return (
    ...
    >
      <div className="modal-header">
        ...
        <IconButton onClick={toggleModal} size="small" className="close">
          <CloseIcon />
        </IconButton>
      </div>
        ...
        >
          {fields?.map((fieldRow, index) => (
            <Grid container spacing={2} className="mb-2" key={index}>
              {fieldRow?.map(
                (
                  { label, type, value, disabled, onChange, options, id, rows },
                  idx
                ) => (
